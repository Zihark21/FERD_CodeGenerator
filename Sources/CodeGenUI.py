...
from Sources.Config import CHAR_LIST, CLASS_LIST, ITEM_LIST, SECTION_HEADER, DESC, CHAR_STATS, CHAR_RANKS, CLASS_STATS, ITEM_STATS, ITEM_DATA, ITEM_BONUS, CODE_DATABASE, VER_LIST
...
class CodeGeneratorGUI:
    ...
    def output_code(self, code):
        ...
        output_label = ttk.Label(message_window, text=code, justify="center", wraplength=400, width=40, anchor="center")
    ...
    def options_tab(self):
        ...
        options_tab.rowconfigure(2, weight=1)
        options_tab.columnconfigure(2, weight=1)
    ...
    def _options_separators(self, options_tab):
        ...
        ttk.Separator(options_tab, orient="vertical").grid(row=0, column=1, rowspan=4, sticky="ns")
    ...
    def _options_info(self, options_tab):
        ...
        options_info.grid(row=2, column=2, padx=10, pady=10, sticky="nsew")

        opt_desc = DESC["Option_Desc"]
        ttk.Label(options_info, text=opt_desc, justify="left", wraplength=1025).grid(row=0, column=0)

    def _options_button(self, options_tab):
        gen_all_button = ttk.Button(options_tab, text="Generate All Codes", command=lambda: self.code_creation(0))
    ...
    def _char_items(self, tab):
        ...
        ttk.Separator(char_items_frame, orient="horizontal").grid(row=1, column=0, columnspan=99, sticky="ew")

        char_desc_frame = ttk.Frame(char_items_frame)
        char_desc_frame.grid(row=2, column=0, sticky='nsew')
    ...
    def _char_button(self, tab):
        char_button = ttk.Button(tab, text="Generate", command=lambda: self.code_creation(1))
    ...
    def class_tab(self):
        ...
        class_tab.rowconfigure(2, weight=1)
        class_tab.columnconfigure(6, weight=1)
    ...
    def _class_info(self, tab):
        ...
        ttk.Label(class_desc_frame, text=class_desc, wraplength=700, justify='left').grid(row=0, column=0)

    def _class_button(self, tab):
        class_button = ttk.Button(tab, text="Generate", command=lambda: self.code_creation(2))
    ...
    def items_tab(self):
        ...
        items_tab.rowconfigure(2, weight=1)
        items_tab.columnconfigure([0,2,4], weight=1, uniform="equal")
    ...
    def _items_button(self, tab):
        item_button = ttk.Button(tab, text="Generate", command=lambda: self.code_creation(3))
    ...
    def append_keycode(self, code):
        key_code = get_keybind_code(self._get_keybinds_data())
    ...
    def generate_database_code(self, sel_code):
        self.type = 'Database'
        version = set_version(self.version.get())
