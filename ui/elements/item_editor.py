...
class ItemEditor(customtkinter.CTkToplevel):
    ...
    def _item_data(self):
        ...
        customtkinter.CTkLabel(frame, text='Item Data', fg_color='gray20', corner_radius=6).grid(columnspan=2, padx=5, pady=5, sticky='nsew')
        ...
        customtkinter.CTkButton(frame, text='Reset', command=reset_data).grid(columnspan=2, padx=5, pady=5, sticky='ew')
    ...
    def _item_stats(self):
        ...
        customtkinter.CTkLabel(frame, text='Item Stats', fg_color='gray20', corner_radius=6).grid(columnspan=2, padx=5, pady=5, sticky='nsew')
        ...
        for i, stat in enumerate(config.item_stats):
            i += 1
            frame.grid_rowconfigure(i, weight=1)
            customtkinter.CTkLabel(frame, text=stat.replace("_", " ")).grid(row=i, column=0, padx=5, pady=5, sticky='nsew')
            entry = customtkinter.CTkEntry(frame, justify='center', width=config.num_entry_width)
            entry.grid(row=i, column=1, padx=5, pady=5)
            self.item_stats.append(entry)

        customtkinter.CTkButton(frame, text='Reset', command=reset_stats).grid(columnspan=2, padx=5, pady=5, sticky='nsew')
    ...
    def _item_bonus(self):
        ...
        customtkinter.CTkLabel(frame, text='Equip Bonuses', fg_color='gray20', corner_radius=6).grid(columnspan=2, padx=5, pady=5, sticky='nsew')
        ...
        for i, bonus in enumerate(config.item_bonus):
            i += 1
            frame.grid_rowconfigure(i, weight=1)
            customtkinter.CTkLabel(frame, text=bonus.replace("_", " ")).grid(row=i, column=0, padx=5, pady=5)
            entry = customtkinter.CTkEntry(frame, justify='center', width=config.num_entry_width)
            entry.grid(row=i, column=1, padx=5, pady=5)
            self.item_bonus.append(entry)
        
        customtkinter.CTkButton(frame, text='Reset', command=reset_bonus).grid(columnspan=2, padx=5, pady=5, sticky='nsew')

    def _item_buttons(self):

        customtkinter.CTkButton(self, text='Generate Item Code', command=lambda: app.App.handleCode(self.root, 'item')).grid(columnspan=3, padx=5, pady=5, sticky='nsew')

        customtkinter.CTkButton(self, text='Close', command=self._close).grid(columnspan=3, padx=5, pady=5, sticky='nsew')